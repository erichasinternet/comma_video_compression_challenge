...
def main():
    ...
    pose_q_br = data_dir / "poseq.bin.br"
    # Short-name fallbacks shave a few bytes of zip header overhead.
    if not model_br.exists(): model_br = data_dir / "b"
    if not mask_br.exists(): mask_br = data_dir / "a"
    if not pose_q_br.exists(): pose_q_br = data_dir / "c"

    # qpose14 packs the three compressed streams into one zip member to save
    # central-directory/header overhead.  Lengths are fixed for this public file.
