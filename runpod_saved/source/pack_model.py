...
def build_qpack(data: dict, quantize_fp16: str = "none") -> bytes:
    ...
    for module_name, rec in source.items():
        ...
        if rec["weight_kind"] == "fp4_packed":
            ...
            if quantize_fp16 == "int8" and np.issubdtype(weight_arr.dtype, np.floating):
    ...
    for name, tensor in data.get("dense_fp16", {}).items():
        arr = tensor_to_array(tensor)
        if quantize_fp16 == "int8" and np.issubdtype(arr.dtype, np.floating):
...
def main():
    ...
    parser.add_argument("--quantize-fp16", choices=["none", "int8"], default="none")
    ...
    report = {"out": str(out), "qpack_bytes": out.stat().st_size, "model_pt_br_bytes": old_size}
    if old_size:
